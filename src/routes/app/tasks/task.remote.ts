...
export const mark_task = command(
	...
	async ({ relation_id, task_id, mark }) => {
		const taskRef = db.collection('relations').doc(relation_id).collection('tasks').doc(task_id);
		...
		await taskRef.update({ marked_at: new Date(), mark } as Partial<Task>);

		const points_added = taskData?.bounty * (mark ? 1 : -1);
		await db
		...
		// Send notification
		const notificationTitle = points_added >= 0 ? 'good puppyyyy!!' : 'bad puppy..';
...
export const create_task = command(
	...
	async ({ relation_id, bounty, done, name }) => {
		...
		const taskRef = db.collection('relations').doc(relation_id).collection('tasks').doc(id);
