<script lang="ts">
	...
	import { init_app_state_context } from './app.svelte';
	import { add_toast } from '$lib/toast.svelte';
	import EditNameModal from './EditNameModal.svelte';

	const app_state = init_app_state_context(get_auth_state());

	let { children } = $props();

	let is_drawer_open = $state(false);
	let is_modal_open = $state(false);
	let edit_name_modal: HTMLDialogElement;

	$effect(() => {
		if (!app_state.auth.loading && !app_state.auth.current) goto(resolve('/login_page'));
	});

	let login_code = $derived(app_state.auth.current?.recon_code);
	let join_code = $derived(app_state.auth.current?.relation.join_code);

	let role = $derived(app_state.auth.current?.role);
	let you_name = $derived(
		role === 'dom' ? app_state.auth.current?.relation.dom_name : app_state.auth.current?.relation.sub_name
	);
	let other_name = $derived(
		role === 'dom' ? app_state.auth.current?.relation.sub_name : app_state.auth.current?.relation.dom_name
	);
	...
	async function do_logout() {
		await app_state.logout();
		is_drawer_open = false;
		goto(resolve('/login_page'));
	}
...
<div class="drawer">
	...
	<div class="drawer-side">
		...
		<div class="menu min-h-full w-80 bg-base-200 p-4">
			...
			{#if app_state.auth.current?.role == 'dom' && app_state.auth.current?.relation.join_code}
			...
			<button onclick={() => (is_modal_open = true)} class="btn mt-2 btn-primary">edit name</button>
