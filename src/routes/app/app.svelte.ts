...
export class App {
	 ...
	 */
	paginated_tasks = $state<ReturnType<typeof firekitCollection<Task & { id: string }>> | null>(
		null
	);
	 ...
	 */
	new_tasks = firekitCollection<Task & { id: string }>(`relations/${this.relation_id}/tasks`, [
	...
	async ensureTasksLoaded() {
		...
		const feed = firekitCollection<Task & { id: string }>(`relations/${this.relation_id}/tasks`, [
			orderBy('created_at', 'desc')
		]);
